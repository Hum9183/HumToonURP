...
namespace HumToon.Editor
{
    ...
    public abstract class BaseShaderGUI : ShaderGUI
    {
        ...
        public virtual void FindProperties(MaterialProperty[] properties)
        {
            ...
            surfaceTypeProp = FindProperty(HumToonProperties.SurfaceType, properties, false);
            blendModeProp = FindProperty(HumToonProperties.BlendMode, properties, false);
            preserveSpecProp = FindProperty(HumToonProperties.BlendModePreserveSpecular, properties, false);  // Separate blend for diffuse and specular.
            cullingProp = FindProperty(HumToonProperties.CullMode, properties, false);
            zwriteProp = FindProperty(HumToonProperties.ZWriteControl, properties, false);
            ztestProp = FindProperty(HumToonProperties.ZTest, properties, false);
            alphaClipProp = FindProperty(HumToonProperties.AlphaClip, properties, false);

            // ShaderGraph Lit and Unlit Subtargets only
            castShadowsProp = FindProperty(HumToonProperties.CastShadows, properties, false);
            queueControlProp = FindProperty(HumToonProperties.QueueControl, properties, false);

            // ShaderGraph Lit, and Lit.shader
            receiveShadowsProp = FindProperty(HumToonProperties.ReceiveShadows, properties, false);
            ...
            emissionMapProp = FindProperty(HumToonProperties.EmissionMap, properties, false);
            emissionColorProp = FindProperty(HumToonProperties.EmissionColor, properties, false);
            queueOffsetProp = FindProperty(HumToonProperties.QueueOffset, properties, false);
        }
        ...
        public virtual void DrawSurfaceOptions(Material material)
        {
            ...
            if ((surfaceTypeProp != null) && ((SurfaceType)surfaceTypeProp.floatValue == SurfaceType.Transparent))
            {
                ...
                if (material.HasProperty(HumToonProperties.BlendModePreserveSpecular))
                {
                    BlendMode blendMode = (BlendMode)material.GetFloat(HumToonProperties.BlendMode);
        ...
        internal static void UpdateMaterialSurfaceOptions(Material material, bool automaticRenderQueue)
        {
            ...
            if (material.HasProperty(HumToonProperties.CastShadows))
            {
                castShadows = (material.GetFloat(HumToonProperties.CastShadows) != 0.0f);
            }
            else
            {
                if (isShaderGraph)
            ...
            // Receive Shadows
            if (material.HasProperty(HumToonProperties.ReceiveShadows))
                CoreUtils.SetKeyword(material, ShaderKeywordStrings._RECEIVE_SHADOWS_OFF, material.GetFloat(HumToonProperties.ReceiveShadows) == 0.0f);
        }

        internal static bool IsOpaque(Material material)
        {
            bool opaque = true;
            if (material.HasProperty(HumToonProperties.SurfaceType))
                opaque = ((BaseShaderGUI.SurfaceType)material.GetFloat(HumToonProperties.SurfaceType) == BaseShaderGUI.SurfaceType.Opaque);
            return opaque;
        }
        ...
        internal static void UpdateMaterialRenderQueueControl(Material material)
        {
            ...
            if (!isShaderGraph)
            {
                material.SetFloat(HumToonProperties.QueueControl, (float)QueueControl.Auto); // Automatic behavior - surface type override
            }
            else
            {
                material.SetFloat(HumToonProperties.QueueControl, (float)QueueControl.UserOverride); // User has selected explicit render queue
            }
        }
        ...
        internal static bool GetAutomaticQueueControlSetting(Material material)
        {
            ...
            if (material.HasProperty(HumToonProperties.QueueControl))
            {
                var queueControl = material.GetFloat(HumToonProperties.QueueControl);
                ...
                automaticQueueControl = (material.GetFloat(HumToonProperties.QueueControl) == (float)QueueControl.Auto);
            }
        ...
        public static void SetMaterialKeywords(Material material, Action<Material> shadingModelFunc = null, Action<Material> shaderFunc = null)
        {
            ...
            if (material.HasProperty(HumToonProperties.CullMode))
                material.doubleSidedGI = (RenderFace)material.GetFloat(HumToonProperties.CullMode) != RenderFace.Front;
            ...
            // Emission
            if (material.HasProperty(HumToonProperties.EmissionColor))
        ...
        {
            if (material.HasProperty(HumToonProperties.SrcBlend))
                material.SetFloat(HumToonProperties.SrcBlend, (float)srcBlend);

            if (material.HasProperty(HumToonProperties.DstBlend))
                material.SetFloat(HumToonProperties.DstBlend, (float)dstBlend);

            if (material.HasProperty(HumToonProperties.SrcBlendAlpha))
                material.SetFloat(HumToonProperties.SrcBlendAlpha, (float)srcBlend);

            if (material.HasProperty(HumToonProperties.DstBlendAlpha))
                material.SetFloat(HumToonProperties.DstBlendAlpha, (float)dstBlend);
        }
        ...
        {
            if (material.HasProperty(HumToonProperties.SrcBlend))
                material.SetFloat(HumToonProperties.SrcBlend, (float)srcBlendRGB);

            if (material.HasProperty(HumToonProperties.DstBlend))
                material.SetFloat(HumToonProperties.DstBlend, (float)dstBlendRGB);

            if (material.HasProperty(HumToonProperties.SrcBlendAlpha))
                material.SetFloat(HumToonProperties.SrcBlendAlpha, (float)srcBlendAlpha);

            if (material.HasProperty(HumToonProperties.DstBlendAlpha))
                material.SetFloat(HumToonProperties.DstBlendAlpha, (float)dstBlendAlpha);
        }

        internal static void SetMaterialZWriteProperty(Material material, bool zwriteEnabled)
        {
            if (material.HasProperty(HumToonProperties.ZWrite))
                material.SetFloat(HumToonProperties.ZWrite, zwriteEnabled ? 1.0f : 0.0f);
        }
        ...
        internal static void SetupMaterialBlendModeInternal(Material material, out int automaticRenderQueue)
        {
            ...
            if (material.HasProperty(HumToonProperties.AlphaClip))
                alphaClip = material.GetFloat(HumToonProperties.AlphaClip) >= 0.5;
            ...
            if (material.HasProperty(HumToonProperties.SurfaceType))
            {
                SurfaceType surfaceType = (SurfaceType)material.GetFloat(HumToonProperties.SurfaceType);
                bool zwrite = false;
                ...
                else // SurfaceType Transparent
                {
                    ...
                    bool preserveSpecular = (material.HasProperty(HumToonProperties.BlendModePreserveSpecular) &&
                                             material.GetFloat(HumToonProperties.BlendModePreserveSpecular) > 0) &&
                ...
                if (material.HasProperty(HumToonProperties.AlphaToMask))
                {
                    material.SetFloat(HumToonProperties.AlphaToMask, alphaToMask ? 1.0f : 0.0f);
                }

                // check for override enum
                if (material.HasProperty(HumToonProperties.ZWriteControl))
                {
                    var zwriteControl = (ZWriteControl)material.GetFloat(HumToonProperties.ZWriteControl);
            ...
            if (material.HasProperty(HumToonProperties.QueueOffset))
                renderQueue += (int)material.GetFloat(HumToonProperties.QueueOffset);
